# SEO README

## Resumen
Este documento explica las recomendaciones SEO propuestas para MARSO - Bebidas & Distribución (Bebidas Germán) con foco local en Hinojosa del Duque (Córdoba) y la comarca de Los Pedroches.

## 1) Meta tags estratégicos
**Title sugerido (≤ 60 caracteres):**
- MARSO | Distribuidor de bebidas en Hinojosa del Duque

**Meta description sugerida (≤ 155 caracteres):**
- Distribuidor mayorista de bebidas en Hinojosa del Duque, Los Pedroches. Marcas líderes y reparto rápido para hostelería.

**Objetivo:** mejorar CTR con palabras clave locales y de intención comercial (distribuidor, mayorista, ubicación).

## 2) Etiqueta canonical
**Sugerida:**
- https://tudominio.com/

**Objetivo:** evitar contenido duplicado y consolidar autoridad.

## 3) Open Graph (redes sociales)
**Propuesto:**
- og:type = website
- og:title = MARSO - Distribuidor de bebidas en Hinojosa del Duque
- og:description = Mayorista de bebidas en Los Pedroches...
- og:url = https://tudominio.com/
- og:image = https://tudominio.com/images/og-marso.jpg
- og:locale = es_ES

**Objetivo:** tarjeta atractiva al compartir en WhatsApp/rrss.

## 4) Schema.org (JSON-LD)
**Tipo:** LocalBusiness + WholesaleStore

**Incluye:**
- Nombre comercial y legal
- Dirección completa en Hinojosa del Duque
- Teléfono
- Horario
- Marcas principales (Mahou, San Miguel, Alhambra, Solán de Cabras)
- URL y zona de servicio (Los Pedroches)

**Objetivo:** mejorar visibilidad en resultados locales y rich results.

## 5) Jerarquía de encabezados (H1-H3)
**Propuesta:**
- Un único H1 con la palabra clave principal
- H2 para servicios (Distribución a Hostelería, Catálogo de Marcas)
- H3 para detalles específicos

**Objetivo:** claridad semántica para buscadores.

## 6) Optimización de imágenes
**Recomendaciones:**
- Alt descriptivo y local en logo del hero y marcas
- Ejemplos:
  - “MARSO Bebidas Germán - distribuidor de bebidas en Hinojosa del Duque”
  - “Marcas distribuidas por MARSO en Los Pedroches”

**Objetivo:** mejorar relevancia local y accesibilidad.

## 7) Accesibilidad (ARIA)
**Sugerencias:**
- aria-label en botón de menú móvil
- aria-label en botón de WhatsApp
- aria-label en botón “Volver arriba”
- aria-label en formulario

**Objetivo:** accesibilidad y mejor puntuación SEO técnica.

## 8) Contenido local (Sobre Nosotros)
**Párrafo sugerido:**
“Desde 1993, MARSO (Bebidas Germán) es referente en distribución mayorista en Hinojosa del Duque y la comarca de Los Pedroches, con servicio cercano y marcas líderes.”

**Objetivo:** reforzar relevancia geográfica.

---

Si quieres, puedo aplicar todo esto directamente en el HTML y ajustar los textos según dominio real, teléfono y dirección exacta.